...
for (( n=$START_N; n<=$END_N; n+=$STEP_N )); do
    ...
    avg_nearmiss_noprune=$(awk "BEGIN {print $sum_nearmiss_noprune / $TRIALS}")
    ...
    echo "$n,RandomDivisible,BacktrackingNoPrune,$avg_divisible_noprune" >> $RESULTS_FILE
    ...
    echo "$n,NearMiss,BacktrackingNoPrune,$avg_nearmiss_noprune" >> $RESULTS_FILE
